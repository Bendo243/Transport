...
import { Upload, Button, Input, Select, DatePicker, Modal, Form, message, UploadFile } from 'antd'; 
...
const ReportIncident: React.FC<ReportIncidentProps> = ({ visible, onOk, onCancel }) => {
  // Define fileList state with type UploadFile[]
  ...
  const handleOk = () => {
    form.validateFields()
      .then(values => {
        ...
        // Logic to handle form values and uploaded files

        form.resetFields();
        setFileList([]);  // Clear file list after submission
        onOk();  // Call the passed onOk handler to close modal
  ...
  // Type newFileList as UploadFile[]
  ...
  return (
    <>
      <Modal
        ...
        footer={[
          <Button key="cancel" onClick={onCancel} className='bottom-10 w-[144px] h-[32px] '>
            Cancel
          </Button>,
          <Button key="submit" type="primary" onClick={handleOk} className='bottom-10 w-[144px] h-[32px] mr-4'>
            Submit
          </Button>,
